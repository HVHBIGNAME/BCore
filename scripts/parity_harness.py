...
def block_map_vanilla(x,z,region,ymin,ymax):
    ...
    data=json.loads(out)
    return {tuple(row[:3]): row[3] for row in data["blocks"]}, data["position"]
...
def main():
    ...
    lines=["# BCore / Vanilla 26.1 parity report", "", f"- Seed: `{SEED}`", "- BCore: standalone release examples; vanilla: `127.0.0.1:25571`", f"- Samples: `{SAMPLES}`; region: `{region}x{region}`", "- BCore was not rebuilt by this harness. Prefer worldgen_snapshot.py for full-height, state-property and biome comparison.", "", "## Terrain height parity", "", "| sample | columns | exact height | match |", "|---|---:|---:|---:|"]
    ...
    lines += ["", f"## Block-name diff (Y={ymin}..{ymax})", "", "| sample | compared cells | equal | different | diff % |", "|---|---:|---:|---:|---:|"]
