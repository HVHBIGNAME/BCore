...
use crate::{block, simplex::{fork_seed_for, JavaRandom, NoiseRegistry}};
...
struct FluidStatus { level: i32, lava: bool }
...
impl<'a> Aquifer<'a> {
    pub fn new(seed: i64, top: i32, water_column: bool, noises: &'a NoiseRegistry) -> Self {
        Self { seed, top, water_column, noises, locations: [((0,0,0),(0,0,0)); 12] }
    }

    /// Resolve a density into the block substance (never returns `None`).
    pub fn substance(&mut self, x: i32, y: i32, z: i32, density: f64) -> u32 {
        if density > 0.0 { return if y < crate::DEEPSLATE_Y { block::DEEPSLATE } else { block::STONE }; }
        ...
        let fluid = if sim12 <= 0.0 { a } else {
            ...
            if density + sim12 * pressure > 0.0 { return if y < crate::DEEPSLATE_Y { block::DEEPSLATE } else { block::STONE }; }
            let sim13 = 1.0 - (d3 - d1) as f64 / 25.0;
            if sim13 > 0.0 && density + sim12 * sim13 * self.pressure(x,y,z,a,c) > 0.0 {
                return if y < crate::DEEPSLATE_Y { block::DEEPSLATE } else { block::STONE };
            }
            a
        };
        if fluid.level != NO_FLUID && y < fluid.level {
            if fluid.lava { block::LAVA } else { block::WATER }
        } else { block::AIR }
    }

    fn nearest(&mut self, x: i32, y: i32, z: i32) -> ((i32,i32,i32),(i32,i32,i32),(i32,i32,i32)) {
        let ax = x.div_euclid(16); let ay = (y + 1).div_euclid(12); let az = z.div_euclid(16);
        let mut v: Vec<((i32,i32,i32), i32)> = Vec::with_capacity(12);
        for dx in 0..=1 { for dy in -1..=1 { for dz in 0..=1 {
    ...
    fn location(&mut self, cell: (i32,i32,i32)) -> (i32,i32,i32) {
        ...
        for &((cx,cy,cz), p) in &self.locations { if (cx,cy,cz)==cell && (cx!=0||cy!=0||cz!=0) { return p; } }
        let named = crate::noise_perlin::java_string_hash("minecraft: aquifer");
        let mut r=JavaRandom::new(named as i64 ^ fork_seed_for(self.seed)
        ...
        let p=(cell.0*16+r.next_int(10) as i32, cell.1*12+r.next_int(9) as i32, cell.2*16+r.next_int(10) as i32);
        for slot in &mut self.locations { if slot.0==(0,0,0) { *slot=(cell,p); break; } }
        p
    }

    fn status(&self, cell: (i32,i32,i32)) -> FluidStatus {
        let (x,y,z)=(cell.0*16,cell.1*12,cell.2*16);
        let global_level=if self.water_column { crate::SEA_LEVEL } else { NO_FLUID };
        let flooded=self.noises.sample("aquifer_fluid_level_floodedness",self.seed,x as f64,y as f64*0.67,z as f64);
        if flooded < -0.15 { return FluidStatus{level:NO_FLUID,lava:false}; }
        let spread=self.noises.sample("aquifer_fluid_level_spread",self.seed,(cell.0 as f64)/1.0,(cell.1 as f64)/1.0,(cell.2 as f64)/1.0)*10.0;
        let spread=(spread/3.0).round() as i32*3;
        let level=(cell.1*12+6+spread).min(self.top-8).min(global_level.max(NO_FLUID));
        let lava=self.noises.sample("aquifer_lava",self.seed,(cell.0 as f64)/4.0,(cell.1 as f64)/4.0,(cell.2 as f64)/4.0).abs()>0.3 && level <= -10;
        FluidStatus{level,lava}
    }

    fn pressure(&self,x:i32,y:i32,z:i32,a:FluidStatus,b:FluidStatus)->f64 {
        if a.lava != b.lava && a.level != NO_FLUID && b.level != NO_FLUID { return 2.0; }
        let diff=(a.level-b.level).abs(); if diff==0 { return 0.0; }
        let avg=(a.level+b.level) as f64*0.5;
        let h=y as f64+0.5-avg;
        let g=(diff as f64/2.0-h.abs())/if h>0.0 {1.5} else {3.0};
        2.0*(g+self.noises.sample("aquifer_barrier",self.seed,x as f64,y as f64*0.5,z as f64))
    }
}

fn dist(p:(i32,i32,i32),x:i32,y:i32,z:i32)->i32 { (p.0-x).pow(2)+(p.1-y).pow(2)+(p.2-z).pow(2) }
